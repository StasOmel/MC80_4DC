...
#ifdef   STFS_MX25UM25645G_FLASH_DRIVER
...
const T_DataFlash_configuration MX25UM25645G_DataFlash_Config =
{
  0,
  MX25UM25645G_SECTOR_SIZE,
  MX25UM25645G_SECTORS_NUM,
  MX25UM25645G_DataFlash_map,
};

static TX_MUTEX                    stfs_mutex;
static TX_EVENT_FLAGS_GROUP        stfs_flag_grp;
static uint8_t                     stfs_driver_initialised = 0;
static uint32_t                    sectors_erase_counters[MX25UM25645G_SECTORS_NUM];
...
/*-----------------------------------------------------------------------------------------------------
  Get pointer to sector memory location
...
uint8_t* STfs_get_sector(uint32_t sector_num)
{
  return (uint8_t *)(MX25UM25645G_DataFlash_map[sector_num].start_adr);
}
...
uint32_t STfs_get_sector_size(uint32_t sector_num)
{
  return MX25UM25645G_SECTOR_SIZE;
}
...
uint32_t STfs_get_sec_num(void)
{
  return MX25UM25645G_SECTORS_NUM;
}

/*-----------------------------------------------------------------------------------------------------
  Get sector erase counter
...
/*-----------------------------------------------------------------------------------------------------
  Wait for synchronization flags

  \param flags
  \param p_actual_flags
  \param timeout_ms
...
/*-----------------------------------------------------------------------------------------------------
  Initialize flash driver for MX25UM25645G
...
uint32_t STfs_flash_driver_init(void)
{
  ...
  // Initialize sector map for MX25UM25645G
  for (uint32_t i = 0; i < MX25UM25645G_SECTORS_NUM; i++)
  {
    MX25UM25645G_DataFlash_map[i].start_adr = MX25UM25645G_FLASH_BASE_ADDR + (i * MX25UM25645G_SECTOR_SIZE);
    MX25UM25645G_DataFlash_map[i].end_adr = MX25UM25645G_DataFlash_map[i].start_adr + MX25UM25645G_SECTOR_SIZE - sizeof(T_stfs_sector_descriptor);
    sectors_erase_counters[i] = 0;
  }

  // Create mutex for API access in multitasking environment
  if (tx_mutex_create(&stfs_mutex, "STFS", TX_INHERIT) != TX_SUCCESS)
  {
    return STFS_ERROR;
  }

  if (tx_event_flags_create(&stfs_flag_grp, "STFS") != TX_SUCCESS)
  {
    tx_mutex_delete(&stfs_mutex);
    return STFS_ERROR;
  }
...
int32_t FlashDriver_get_flash_mem_params(void **pdev_map, uint32_t *number_of_sectors)
{
  *pdev_map = (void *)MX25UM25645G_DataFlash_Config.sect_map;
  *number_of_sectors = MX25UM25645G_DataFlash_Config.flash_sect_num;
  return 0;
}
...
int32_t FlashDriver_erase_sector(uint32_t sector)
{
  ...
  if (sector >= MX25UM25645G_SECTORS_NUM)
  {
    return STFS_SECTOR_ERASE_ERROR1;
  }

  sector_address = MX25UM25645G_FLASH_BASE_ADDR + (sector * MX25UM25645G_SECTOR_SIZE);
...
int32_t FlashDriver_program_pages(uint32_t addr, uint32_t data_size, uint8_t *buf, uint8_t *prgwrd_buf, uint8_t *prog_word_cnt)
{
  ...
  if (rem != 0)
  {
    // Address not aligned to FLASH_WORD_SIZE boundary
    curr_addr = mod * STFS_FLASH_WORD_SIZE;
    sz = STFS_FLASH_WORD_SIZE - rem;

    if (data_size < sz)
    {
      // Not enough data to complete the word, store in buffer
      memcpy(&prgwrd_buf[rem], buf, data_size);
      *prog_word_cnt += data_size;
      return STFS_OK;
    }
...
/*-----------------------------------------------------------------------------------------------------
  Program aligned pages to flash
...
int32_t FlashDriver_read_data(uint32_t addr, uint32_t sz, uint8_t *buf)
{
  // In memory-mapped mode, we can read directly from the address
  memcpy(buf, (void *)addr, sz);
  return STFS_OK;
}

#endif // STFS_MX25UM25645G_FLASH_DRIVER
